...
;;   - <nop> used in a heading gets marked up
...
(defun emacs-wiki-maybe (&optional check-only)
  ...
  (let ((d emacs-wiki-directories)
	...
	yes)
    (while d
      (if (string= here (expand-file-name (if (consp (car d))
					      (caar d)
					    (car d))))
...
(defun emacs-wiki-find-file (wiki-name &optional command)
  ...
  (let ((wiki (assoc wiki-name emacs-wiki-file-alist)))
	...
	(funcall (or command 'find-file) (cdr wiki))
      (let* ((wiki-dir (or (emacs-wiki-maybe t)
...
affect the match data results.  Use the shy-group construct, \\(?:\\),
if you need to group.  This is so that the final rule, for URLs or
WikiNames, can be optimized."
  :type '(repeat
...
(defcustom emacs-wiki-markup-tags
  '(("example" t nil emacs-wiki-example-tag)
    ("verbatim" t nil emacs-wiki-verbatim-tag)
    ("verse" t nil emacs-wiki-verse-tag)
    ("redirect" t t emacs-wiki-redirect-tag)
    ("nop" nil nil emacs-wiki-nop-tag)
    ("contents" nil t emacs-wiki-contents-tag)
    ("lisp" t nil emacs-wiki-lisp-tag))
...
The tags themselves are always deleted, before the function is called.
The function is called with two arguments, the beginning and end of
the region.  If properties are allowed, they are passed as a third
argument in the form of an alist.

Point is always at the beginning of the region when the function is
called.  Wherever point is when the function finished is where tag
markup will resume.  A last note: The `end' argument to the function
is a marker.
...
(defcustom emacs-wiki-publishing-markup
  `(;; change the displayed title or the stylesheet for a given page
    ...
    [,(concat "\\(</?" emacs-wiki-block-groups-regexp ">\\)\\s-*</p>") 0 "\\1"]
...
(defun emacs-wiki-markup-custom-tags ()
  (let ((tag-info (assoc (match-string 1) emacs-wiki-markup-tags)))
    (when tag-info
	  ...
	  (let ((attrstr (match-string 2)))
	    (while (and attrstr
	      ...
	      (let ((attr (cons (downcase
				...
				(match-string-no-properties 2 attrstr))))
		(setq attrstr (replace-match "" t t attrstr))
		(if attrs
		    (nconc attrs (list attr))
		  (setq attrs (list attr)))))))
	(if (and (cadr tag-info)
		 (not closed-tag)
		 (search-forward (concat "</" (car tag-info) ">") nil t))
	    (delete-region (match-beginning 0) (point)))
	(setq end (point-marker))
	(delete-region start beg)
	(goto-char start)
	(if (nth 2 tag-info)
	    (funcall (nth 3 tag-info) start end attrs)
	  (funcall (nth 3 tag-info) start end)))
      nil)))

(defun emacs-wiki-example-tag (beg end)
  (insert "<pre>")
  (setq beg (point))
  (goto-char end)
  (setq end (point))
...
(defun emacs-wiki-verbatim-tag (beg end)
  (goto-char end)
  (add-text-properties beg end '(rear-nonsticky (read-only)
						read-only t)))
...
(defun emacs-wiki-nop-tag (beg end)
...
(defun emacs-wiki-lisp-tag (beg end)
  (save-excursion
